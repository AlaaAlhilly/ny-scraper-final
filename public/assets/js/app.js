$(document).ready(function() {
  $(".scrape").click(function(e) {
      ...
      $.get("/api/get").then(function(data) {
          $(".articles").remove();
          $.get("/").then(function(){
              bootbox.alert("<h3 class='text-center m-top-80'>" + data.msg + "<h3>", function(result) {
                location.reload()
              });
          });
      });
  ...
  $('.saved-buttons').on('click',  function () {
      ...
      $.get("/notes/" + art_id, function(data){
          ...
          for(var i = 0; i<data.note.length; i++) {
              var button = ' <a href=/delete/' + data.note[i]._id + '><i class="pull-right fa fa-times fa-2x deletex" aria-hidden="true"></i></a>';
